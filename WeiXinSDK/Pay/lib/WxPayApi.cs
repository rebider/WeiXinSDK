    ...
    public class WxPayApi
    {
        ...
        */
        public static WxPayData Micropay(WxPayData inputObj, int timeOut = 10)
        {
            ...
            inputObj.SetValue("spbill_create_ip", WeiXin.appConfig.IP);//终端ip
            inputObj.SetValue("appid", WeiXin.appConfig.APP_ID);//公众账号ID
            inputObj.SetValue("mch_id", WeiXin.appConfig.MCHID);//商户号
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            {
                inputObj.SetValue("notify_url", WeiXin.appConfig.NOTIFY_URL);//异步通知url
            }
	    ...
	    */
	    ...
	    {
		    //如果不需要进行上报
		    if(WeiXin.appConfig.REPORT_LEVENL == 0)
            {
			    return;
		    } 

		    //如果仅失败上报
		    if(WeiXin.appConfig.REPORT_LEVENL == 1 && inputObj.IsSet("return_code") && inputObj.GetValue("return_code").ToString() == "SUCCESS" &&
	    ...
	    */
	    public static WxPayData Report(WxPayData inputObj, int timeOut = 1)
	    {
		    ...
		    inputObj.SetValue("appid",WeiXin.appConfig.APP_ID);//公众账号ID
		    inputObj.SetValue("mch_id",WeiXin.appConfig.MCHID);//商户号
		    inputObj.SetValue("user_ip",WeiXin.appConfig.IP);//终端ip
        ...
        */
        ...
        {
            var ran = new Random();
            return string.Format("{0}{1}{2}", WeiXin.appConfig.MCHID, DateTime.Now.ToString("yyyyMMddHHmmss"), ran.Next(999));
        }
